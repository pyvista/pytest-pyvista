...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers",
        "skip_mac(machine=None, reason=...): skip the test on macOS; if machine is given (e.g. 'arm64') only skip when platform.machine() matches.",
    )
    config.addinivalue_line(
        "markers",
    ...
    config.addinivalue_line(
        "markers",
        "needs_vtk_version(*version, at_least=None, less_than=None, reason=...): skip the test "
        "unless the running VTK version satisfies the given bound.",
    )
