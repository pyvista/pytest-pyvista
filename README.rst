...
* Use ``--include_vtksz`` to include tests for interactive plots which are generated by
  ...
  and rendered with the window size set by ``--vtksz_window_size``, or the size of its
...
* Use ``--vtksz_window_size`` to render every ``vtksz`` file at a fixed window size,
  given as ``'WIDTH,HEIGHT'``. When it is not set, each file is rendered at the size of
  its corresponding static image, and an error is raised for any ``vtksz`` file which
  does not have one.
