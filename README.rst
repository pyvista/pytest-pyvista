...
* Use ``--max_image_size`` to specify a single integer which limits the dimensions
  of generated images to this value or smaller. By default, images have dimensions
  ``(1024, 768)`` as determined by the ``Plotter.window_size`` or ``pyvista.global_theme.window_size``.
  If this option is used, the output is resized while preserving the aspect ratio.
  Images are only resized to be smaller, and are not enlarged.
