...
This writes ``image_test_report/index.html`` relative to `pytest root path`_, cataloguing
every generated image comparison from that run, not just the failures, with the
cached baseline, the generated render, the pixel difference between them, and the error
value measured against its threshold. The report reflects only the images generated by
the run that wrote it: it is regenerated, not accumulated, each time.

Writing the report never fails the run. If the report directory cannot be written to (a
read-only filesystem, a full disk), the run degrades to a warning line on the terminal;
the exit code and the rest of the terminal summary are unaffected.

.. note::
   The summary report cannot be used with ``--doc_mode``. Passing both
   ``--summary_html`` and ``--doc_mode`` on the command line is rejected outright.
   Configuring ``summary_html`` through a config file while ``--doc_mode`` is active is
   not an error, but produces no report.

Each image is given one of six statuses:

* ``passed``: the render matched the baseline.
* ``warned``: the error exceeded the warning threshold but not the error threshold, or
  the render matched one of a test's alternate cache images rather than the first. The
  test passed, but the image is worth a look.
...
approved individually in the report with a checkbox on the card, or all ``new`` images
at once with **Accept all new**. Approving is reversible: ticking and unticking
updates the count freely, and **Export approvals** downloads a snapshot of what is
currently ticked as ``approvals.json``. Apply it with the ``pytest-pyvista-approve``
console script:

.. code-block:: bash

    # copy approved images to a staging directory for a second look (default)
    pytest-pyvista-approve approvals.json --image_cache_dir tests/image_cache

    # or write them straight into the image cache
    pytest-pyvista-approve approvals.json --target cache --image_cache_dir tests/image_cache

``--image_cache_dir`` tells the CLI where your project's real image cache is. It defaults
to ``image_cache_dir``, matching ``pytest-pyvista``'s own default, and is independent
ground truth: the manifest's own record of where the cache was is only ever *compared*
against this value, never trusted directly. If your project's cache lives anywhere other
than ``./image_cache_dir`` relative to where you run the command, you must pass
``--image_cache_dir`` explicitly: omitting it does not fall back to whatever directory
the test run actually used, it silently checks the wrong location and every approval is
rejected.
...
* ``--target {staging,cache}`` chooses the destination. ``staging`` (the default) copies
  into ``--staging_dir`` (``approved_images`` by default; pass ``--staging_dir .`` to
  stage into the current directory) for a second look before anything touches the cache.
...
* ``--force`` relaxes only the check that the manifest's recorded cache directory
  ...
  not widen where files may be written, so if your cache is not at the default location,
  ``--force`` will not make a failing run succeed; pass the correct
  ``--image_cache_dir`` instead.

* ``--dry-run`` prints the planned copies without performing them.

.. note::
   ``--image_cache_dir`` and ``--staging_dir`` are both rejected if left empty,
   whitespace-only, or set to the filesystem root.

.. note::
   ``pytest-pyvista-approve``'s directory flags mix naming conventions on purpose,
   matching how they are written elsewhere in this plugin: ``--staging_dir``,
   ``--generated_image_dir`` and ``--image_cache_dir`` are snake_case, while
   ``--dry-run`` is kebab-case.
...
for reference with the reason they were pre-approved, and have no checkbox: nothing would
be removed from the cache by unticking them.

Summary report flags
=====================
* ``--summary_html`` enables report generation.

* ``--summary_html_dir <DIR>`` sets the report directory, relative to `pytest root
  path`_. Defaults to ``image_test_report``. Setting it enables the report even without
  ``--summary_html``.

* ``--summary_html_include <STATUSES>`` restricts which statuses are written to the
  report, as a comma-separated list, e.g. ``failed,warned,new``. All six statuses are
  included by default.

* ``--summary_html_max_image_size <N>`` limits the longest edge of the images shown
  inline on each card. Defaults to ``400``.

* ``--summary_html_full_size <MODE>`` controls which records also keep a full-resolution
  copy on disk, linked from the card. One of ``none``, ``failing`` (the default, meaning
  any non-passing record) or ``all``.
...
* ``--summary_html_embed`` produces a single ``index.html`` with every image inlined as a
  ...
  .. note::
     ...
     URI, so the full-size files end up orphaned. This is deliberate: embedding
     full-resolution images into the page would defeat the size budget that embedding
     exists to respect.
