...
============================
The dimensions of images generated by tests is determined by the ``Plotter.window_size``
attribute which can be set globally using ``pyvista.global_theme.window_size``. By
default, images have dimensions ``(1024, 768)``.

E.g., use a fixture to set dimensions to ``(400, 300)`` consistently tobetween tests.
...
Setting ``window_size`` affects generated images from both regular unit tests and
documentation tests.

- For the unit tests, the generated image dimensions match the window size exactly,
  since this modifies the plotter's window size directly at the source.

- For the documentation tests, however, the window size represents an upper bound,
  and the generated images are only `resized` to fit within the window size specified
  inside of ``conftest.py``. This is because the documentation images were `already`
  plotted by the PyVista plot directive during the documentation build, and therefore
  the window size can no longer be modified directly.

  .. note::

    The window size for the documentation build itself is set inside of ``conf.py``
    for Sphinx, rather than ``conftest.py`` for Pytest. It is recommended for the
    plotter's window size in both ``conf.py`` (for building) and ``conftest.py``
    (for testing) to use the same aspect ratio.
