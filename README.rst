...
* Use ``--include_vtksz`` to include tests for interactive plots which are generated by
  ...
  and rendered with the window size set by ``--window_size``, or the size of its
  corresponding static image. A screenshot is captured, then compared with the cached
  image like normal.
...
* Use ``--window_size`` to set the plot window size, given as ``'WIDTH,HEIGHT'``. It
  overrides ``pyvista.global_theme.window_size``, and every ``vtksz`` file is rendered
  at this size. When it is not set, each ``vtksz`` file is rendered at the size of its
  corresponding static image, or the theme's window size if it has none.
